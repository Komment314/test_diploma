...
def create_opt_informed_rrt_star(env: SceneUR10WithObs, start, prepared_tree, delta, cut_radius, max_iters=1000, max_time=60):
    ...
    optimizer = PathOptimizer(env, delta * 4, env.obs_collection)
    ...
    while not (time.time() - begin_time > max_time and iterations > max_iters):
        ...
        if connection == True:
            ...
            c_best = min(c_best, added_nodes[goal_index].get_cost())
            ellipse_volume = UNIT_BALLS_VOLUMES[env.n_dims] * (c_best / 2) * ((np.sqrt(c_best ** 2 - c_min ** 2) / 2) ** (env.n_dims - 1))
        ...
        # Optimization
        if iters_without_opt >= 300 and iters_done == False:
            if opt_path_cost < c_best - 1e-4:
                result, x_1, x_1_cost = optimizer.optimize(start, goal, best_opt_path, 1500, delta * 7, True, 30)
            else:
                result, x_1, x_1_cost = optimizer.optimize(start, goal, optimizer.get_path(added_nodes, goal_index), 1500, delta * 7, True, 30)
            if result == True:
                if x_1_cost < opt_path_cost:
                    opt_path_cost = x_1_cost
                if x_1_cost < c_best:
                    best_opt_path = x_1
                    c_best = x_1_cost
            iters_without_opt = 0
    ...
    return added_nodes, goal_index, np.vstack([start, best_opt_path, goal]) if best_opt_path is not None else None, np.array(stats)